...
def gh_put(filename, obj, sha, message):
    ...
    try:
        ...
        print(f"  (continuing with local fallback only)")
        return  # Don't exit, let MCP push handle it
...
def main():
    ...
    print("Publishing …")
    # Save locally before pushing (for MCP fallback)
    import json as _json
    with open("data.json.out", "w", encoding="utf-8") as _f:
        _json.dump(new_data, _f, ensure_ascii=False, indent=2)
    with open("seen.json.out", "w", encoding="utf-8") as _f:
        _json.dump(new_seen, _f, ensure_ascii=False, indent=2)
    print("  Saved local copies: data.json.out seen.json.out")
