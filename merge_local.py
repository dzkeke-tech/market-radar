#!/usr/bin/env python3
"""Local variant: runs same merge logic but saves output files instead of pushing to GitHub."""
...
def main():
    ...
    # Save locally
    out_data_path = os.path.join(here, "out_data.json")
    out_seen_path = os.path.join(here, "out_seen.json")
    with open(out_data_path, "w", encoding="utf-8") as f:
        json.dump(new_data, f, ensure_ascii=False, indent=2)
    with open(out_seen_path, "w", encoding="utf-8") as f:
    ...
    print(f"  Saved to out_data.json and out_seen.json")
    print(f"  data_sha={data_sha}")
    print(f"  seen_sha={seen_sha}")
    if dropped:
        print(f"  {len(dropped)} dropped by date-validation")
